...
import { db, typesTable, pokemonTypesTable } from "@workspace/db";
...
router.get("/types", async (_req, res): Promise<void> => {
  const types = await db.select().from(typesTable).orderBy(typesTable.name);
  res.json(types.map((t) => ({ id: t.id, name: t.name, color: t.color })));
});

// GET /types/:name
router.get("/types/:name", async (req, res): Promise<void> => {
  const name = Array.isArray(req.params.name) ? req.params.name[0] : req.params.name;
  const [t] = await db.select().from(typesTable).where(ilike(typesTable.name, name)).limit(1);
  if (!t) {
    res.status(404).json({ error: "Type not found" });
    return;
  }
  const [countRow] = await db
    .select({ count: sql<number>`count(distinct ${pokemonTypesTable.pokemonId})` })
    .from(pokemonTypesTable)
    .where(eq(pokemonTypesTable.typeId, t.id));

  res.json({
    id: t.id,
    name: t.name,
    color: t.color,
    effectiveness: typeChart[t.name] ?? { doubleDamageTo: [], halfDamageTo: [], noDamageTo: [], doubleDamageFrom: [], halfDamageFrom: [], noDamageFrom: [] },
    pokemonCount: Number(countRow.count),
  });
});
